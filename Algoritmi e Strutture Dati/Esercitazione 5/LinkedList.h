...
class LinkedList : public List<TypeElement, Node<TypeElement>* >
{
    public:
        ...
        unsigned int getLengthList(void);
        void reverse(void);
...
template <class TypeElement>
unsigned int LinkedList<TypeElement>::getLengthList(void)
{
...
template <class TypeElement>
void LinkedList<TypeElement>::reverse(void)
{
    position first = this -> begin();
    position last = this -> begin();
    unsigned short length = this -> getLengthList();
    unsigned short i = 0;
    TypeElement temp;
    ...
    while(i < (length / 2))
    {
        temp = this -> read(first);

        this -> write(first, this -> read(last));
        this -> write(last, temp);
...
template <class TypeElement>
bool LinkedList<TypeElement>::palindrome(void)
{
    ...
    while(i < (length / 2) && palindrome)
    {
        if(this -> read(first) != this -> read(last))
        {
            palindrome = false;
        }
