		...
		switch(menu_choice)
		{
			case add_driver:
				...
				if(addStruct(DRIVERS_FILE_PATH, TRAVELS_FILE_PATH, &id_drivers, DRIVER))
				{
					...
					{
						printfError("\n\nA critical error has occurred during the updating of the driver's ID, please delete the following directory: \"Files\"");
						flag = false;
					}
				}
				else
			...
			case add_travel:
				...
				if(addStruct(DRIVERS_FILE_PATH, TRAVELS_FILE_PATH, &id_travels, TRAVEL))
				{
					...
					{
						printfError("\n\nA critical error has occurred during the updating of the travel's ID, please delete the following directory: \"Files\"");
						flag = false;
					}
				}
				else
			...
			case sort_drivers:
				...
				if(sorting_seconds)
				{
					printf("\n\nThe drivers have been sorted in %.2lf seconds", sorting_seconds);
				}
				else
				{
					if(getNumberRecord(DRIVERS_FILE_PATH, sizeof(Driver_t)) == ALREADY_SORTED)
					{
						printf("\n\nOne driver means that the file is already sorted.");
					}
					else
					{
			...
			case sort_travels:
				...
				if(sorting_seconds)
				{
					printf("\n\nThe travels have been sorted in %.2lf seconds", sorting_seconds);
				}
				else
				{
					if(getNumberRecord(TRAVELS_FILE_PATH, sizeof(Travel_t)) == ALREADY_SORTED)
					{
						printf("\n\nOne travel means that the file is already sorted.");
					}
					else
					{
